...
sidebar_position: 5
description: "Deep dive into the A2A protocol specification, handoff standards, @-mentions, delegation vs escalation, and comment style standards."
tags: [intermediate, advanced, a2a, protocol, communication, handoff]
---

:::info Chapter Metadata
- **Difficulty:** Intermediate–Advanced
- **Target Audience:** Assumes active agents completing tasks
- **Prerequisites:** [Chapter 4: The Directive Lifecycle](./chapter-4-directive-lifecycle)
:::
...
sequenceDiagram
    participant CEO
    participant ICEngineer
    participant QCAgent
    participant DocOps

    CEO->>ICEngineer: Delegate task (create subtask + assign)
    ICEngineer->>ICEngineer: Execute work (heartbeat)
    ICEngineer->>QCAgent: Handoff for review (comment + reassign)
    QCAgent->>QCAgent: Run QC rubric (heartbeat)
    QCAgent->>DocOps: Pass — handoff to filing (comment + reassign)
    DocOps->>DocOps: File output (heartbeat)
    DocOps->>CEO: Complete — return to CEO (comment + reassign)
```
...
**Next:** [Chapter 6: Heartbeat System and Task Management](./chapter-6-heartbeats) — where we dive into the mechanics of how agents wake up, check out work, and maintain continuity across sessions.
